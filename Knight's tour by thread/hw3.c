...
typedef struct dead_end_board {
	char** board[limit]; // the three int can be changed 
	int num;
	int covered[limit];
} Board;
...
void print_board( Board* my_b, int k, int m, int n ){
	if (my_b){
		for( int i = 0; i < my_b->num; i++){
			if ( my_b-> covered[i] >= k){
				print_vec( my_b-> board[i], m, n);
			}
		}
	}
}
...
void* dojob(void* arg){
	...
	if( mp1.cur_cov <= mp1.m * mp1.n ){
			...
			p(&mutex);
				(mp1.my_board)->board[mp1.my_board->num] = mp1.current;
				(mp1.my_board)->covered[mp1.my_board->num] = mp1.cur_cov;
				(mp1.my_board)->num++;
				if((mp1.my_board)->num >= limit){
					fprintf(stderr, "Not enough sapce, exit\n" );
					exit(1);
				}
...
int main(int argc, char const *argv[])
{
	...
	// create board
	Board* my_board = malloc(sizeof(Board));
	(*my_board).num = 0;
	...
	printf("THREAD %u: Best solution found visits %d squares (out of %d)\n", (unsigned int)pthread_self(),
	 max_square, row*col);
	p(&mutex);
	print_board( my_board, kval, row, col );
	v(&mutex);
	// free dynamic memeory 
	p(&mutex);
	for( int i = 0; i < my_board->num; i++){
		free_vec( & my_board->board[i], row, col );
	}
	v(&mutex);
	free(my_board);
